...
export default function AddObjekWisata({ navigation }) {
  ...
  const [jamBuka, setJamBuka] = useState(Array.from({ length: 7 }, () => ({ buka: '', tutup: '' })))
  const [htm, setHtm] = useState(Array.from({ length: 7 }, () => ({ dewasa: '', anak: '' })))
  const [samePrice, setSamePrice] = useState(Array.from({ length: 7 }, () => ({ same: true })));
  ...
  const [openEveryday, setOpenEveryday] = useState(true)
  const [samePriceDaily, setsamePriceDaily] = useState(true);
  ...
  useEffect(() => {
    if (openEveryday) {
      setJamBuka(Array.from({ length: 7 }, () => ({ buka: '', tutup: '' })))
    }
  }, [openEveryday]);
      ...
      (
        ...
        >
          ...
          >
            ...
            <Text style={{ color: colors.black, fontSize: 16, fontWeight: '700' }}>Nama Objek Wisata<Text style={{ color: colors.red }}>*</Text></Text>
            <TextInput
              style={{
          ...
          <TouchableOpacity
            ...
            onPress={async () => {
              if (namaObjek !== '' && alamat !== '' && latitude !== null && longitude !== null && deskripsi !== '') {
                ...
                if (!areAllPropertiesEmpty && isAnyPropertyPartiallyEmpty) {
                  ...
                  try {
                    if (imageTemp.length !== 0) {
                      downloadUrls = await Promise.all(imageTemp.map(async (image) => {
                        const downloadUrl = await uploadImage(image.uri);
                        return { uri: downloadUrl };
                      }));
                    }
                    await firestore().collection('node').add({
                      nama: namaObjek,
                      latitude: latitude,
                      longitude: longitude,
                      tipe: 1,
                    ...
                    ToastAndroid.show('Berhasil menambah objek wisata', ToastAndroid.SHORT)
                ...
                // if (jamBuka.every(item => item.buka === '' && item.tutup === '') && !(jamBuka.some(item => item.buka === '' && item.tutup !== '') && jamBuka.some(item => item.buka !== '' || item.tutup === ''))) {
                //   console.log("delta", jamBuka.some(item => item.buka === '' && item.tutup === ''));
                // setLoading(true)
                // let downloadUrls = []
                // try {
                //   if (imageTemp.length !== 0) {
                ...
                // }
                // } else {
                //   console.log("kok salah mulu", jamBuka.some(item => item.buka === '' && item.tutup === ''));
                //   ToastAndroid.show('Informasi harap dilengkapi', ToastAndroid.SHORT)
                // }
              } else {
                ToastAndroid.show('Informasi harap dilengkapi', ToastAndroid.SHORT)
